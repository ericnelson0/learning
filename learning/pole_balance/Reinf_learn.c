...
void upd_prev_pole(Polesys *p) {
	...
	prev_pole.theta_dot[0] = p->theta_dot[0];
	prev_pole.theta_dot[1] = p->theta_dot[1];
	prev_pole.theta_ddot[0] = p->theta_ddot[0];
	prev_pole.theta_ddot[1] = p->theta_ddot[1];
	prev_pole.pos[0] = p->pos[0];
	prev_pole.pos[1] = p->pos[1];
	prev_pole.pos[2] = p->pos[2];
	prev_pole.vel[0] = p->vel[0];
	prev_pole.vel[1] = p->vel[1];
	prev_pole.vel[2] = p->vel[2];
	prev_pole.acc[0] = p->acc[0];
...
/*                  explore : the number of exploration actions taken in    */
...
int *explore;
{
 ...
 // trial in the next time step once the pendulum has fallen
  ...
  Q_table[state[0]][state[1]] += alpha * (freq_table[state[0]][state[1]]) * (reward + gamma * max_diff);
